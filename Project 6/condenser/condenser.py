...
from condenser.simulation_utils.DP_air import DP_air
from condenser.simulation_utils.airsidehtc_chang_wang import airsidehtc_chang_wang
from condenser.epsilon_NTU import epsilon_NTU
...
class condenser:
    ...
    '''
    def __init__(self,T_ri,T_sat_ri,m_dot_ref,T_ai,P_ai,Vel_face_ai,N_segment):
        self.fld='R1234yf'
        # -------------- Geometry Info ------------- #
        ...
        self.vol_dot_air = Vel_face_ai*self.A_front                         # [m^3/s]
        self.T_ai = T_ai                                                    # [C]
        self.P_ai = P_ai                                                    # [kPa]
...
def result_organizer(result_tmp):
    ...
    variables in result_tmp:
        T_ro,P_ro,x_ro,T_ao,P_ao,Q_tot,T_ro_dict,P_ro_dict,x_ro_dict,Q_dist_dict,T_r_dist_dict,T_a_dist_dict,
        P_r_dist_dict,P_a_dist_dict,DELTAP_air_dist_dict,DELTAP_ref_dist_dict,x_r_dist_dict,htc_ref_dist_dict
    '''
    result = {}
    variables = ['T_ro','P_ro','x_ro','T_ao','P_ao','Q_tot','T_ro_dict','P_ro_dict','x_ro_dict',\
                 'Q_dist','T_r_dist','T_a_dist','P_r_dist','P_a_dist',\
                 'DELTAP_air_dist','DELTAP_ref_dist','x_r_dist','htc_ref_dist']
    for i, value in enumerate(result_tmp):
        if i<=8:
            result[variables[i]] = (value)
        else:
            result[variables[i]] = np.concatenate((value[0],value[1],value[2],value[3]))
...
def condenser_system(self,htc_correlation):
    ...
    Q_tot = np.sum(Q_dist_dict[0])*self.N_tube[0] + np.sum(Q_dist_dict[1])*self.N_tube[1] + np.sum(Q_dist_dict[2])*self.N_tube[2] + np.sum(Q_dist_dict[3])*self.N_tube[3]

    result_tmp = [T_ro,P_ro,x_ro,T_ao,P_ao,Q_tot,T_ro_dict,P_ro_dict,x_ro_dict,Q_dist_dict,\
                  T_r_dist_dict,T_a_dist_dict,P_r_dist_dict,P_a_dist_dict,DELTAP_air_dist_dict,\
                  DELTAP_ref_dist_dict,x_r_dist_dict,htc_ref_dist_dict]
